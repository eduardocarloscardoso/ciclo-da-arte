...
async function montarModuloPipelineB2C(containerId) {
  ...
  var ST = { leads: [], canais: [], statusCrm: [], editId: null, dragId: null, transicao: null };
  ...
  try {
    var res = await Promise.all([cdaCarregarLeadsB2C(), cdaCarregarCanais(), cdaCarregarStatusCrm()]);
    ST.leads = res[0]; ST.canais = res[1]; ST.statusCrm = res[2];
  } catch (err) {
    console.error(err);
