...
async function montarModuloClientes(containerId, opts) {
  ...
  host.innerHTML =
    ...
    '<div class="tw">' +
      ...
      '<div class="ts"><table>' +
        '<thead><tr><th>Canal de Venda</th><th>Cidade / UF</th><th>Nome</th><th>Sexo</th><th>E-mail</th><th>Telefone</th><th>Tipo</th><th>Tipo Comercial</th>' + (editavel ? '<th></th>' : '') + '</tr></thead>' +
  ...
  function render() {
    ...
    tb.innerHTML = pg.map(function (c) {
      ...
      return '<tr>' +
        ...
        '<td>' + tipo + '</td>' +
        '<td>' + tipoComBadge + '</td>' +
        (editavel ? '<td><button class="btn sm" data-edit="' + c.id + '">✏</button> <button class="btn sm" data-del="' + c.id + '">🗑</button></td>' : '') +
        '</tr>';
    }).join('') || '<tr><td colspan="' + (editavel ? 9 : 8) + '" style="text-align:center;color:var(--muted);padding:20px">Nenhum cliente encontrado com os filtros atuais.</td></tr>';
  ...
  if (editavel) host.querySelector('#cdacli-btn-novo').addEventListener('click', function () { abrirModal(null); });
